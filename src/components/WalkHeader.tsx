...
export default function WalkHeader() {
  ...
  return (
    <header className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 lg:gap-4 mb-6 lg:mb-8 pb-4 lg:pb-6 border-b theme-border w-full">
      <Link 
        href="/" 
              ...
              <Link
                ...
                className={`px-2 py-2.5 sm:px-3 sm:py-2.5 lg:px-4 lg:py-3 rounded-md font-medium transition-all duration-200 text-sm lg:text-base whitespace-nowrap min-h-[44px] flex items-center ${
                  path.replace(/\/$/, '') === walkItem.href.replace(/\/$/, '')
                    ...
                    : 'theme-text-muted hover:theme-text-primary hover:opacity-80 hover:theme-bg-secondary transition-colors'
                }`}
              >
                {walkItem.title}
              </Link>
